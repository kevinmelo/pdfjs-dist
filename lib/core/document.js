...
let _annotation = require('./annotation.js');

let _crypto = require('./crypto.js');

let _parser = require('./parser.js');

let _operatorList = require('./operator_list.js');

let _evaluator = require('./evaluator.js');
...
function isAnnotationRenderable(annotation, intent) {
  return (
    (intent === 'display' && annotation.viewable) || (intent === 'print' && annotation.printable)
  );
}
...
class Page {
  ...
  _getInheritableProperty(key, getArray = false) {
    const value = (0, _coreUtils.getInheritableProperty)({
      dict: this.pageDict,
      key,
      getArray,
      stopWhenFound: false
  ...
  get resources() {
    return (0, _util.shadow)(
      this,
      'resources',
      this._getInheritableProperty('Resources') || _primitives.Dict.empty
    );
  }
  ...
  getOperatorList({ handler, sink, task, intent, renderInteractiveForms }) {
    ...
    const pageListPromise = dataPromises.then(([contentStream]) => {
      const opList = new _operatorList.OperatorList(intent, sink, this.pageIndex);
    ...
    return Promise.all([pageListPromise, this._parsedAnnotations]).then(function([
      pageOpList,
      annotations
    ]) {
      ...
      for (const annotation of annotations) {
        if (isAnnotationRenderable(annotation, intent)) {
          opListPromises.push(
            annotation.getOperatorList(partialEvaluator, task, renderInteractiveForms)
          );
        }
      }
    ...
    const parsedAnnotations = this.pdfManager.ensure(this, 'annotations').then(() => {
      ...
      for (let i = 0, ii = annotationRefs.length; i < ii; i++) {
        annotationPromises.push(
          _annotation.AnnotationFactory.create(
            this.xref,
            annotationRefs[i],
            this.pdfManager,
            this.idFactory
...
class PDFDocument {
  ...
  get linearization() {
    ...
    try {
      ...
      if (err instanceof _coreUtils.MissingDataException) {
        throw err;
      }
  ...
  get startXRef() {
    ...
    if (this.linearization) {
      ...
      if (found) {
        ...
        do {
          ch = stream.getByte();
        } while ((0, _coreUtils.isWhiteSpace)(ch));
      ...
      for (const key of infoDict.getKeys()) {
        ...
        if (DocumentInfoValidators[key]) {
          ...
          if ((0, _util.isString)(value)) {
            ...
            (0, _primitives.isName)(value) ||
            (0, _util.isNum)(value) ||
            (0, _util.isBool)(value)
          ) {
            customValue = value;
          } else {
  ...
  get fingerprint() {
    ...
    if (
      ...
      idArray[0] &&
      (0, _util.isString)(idArray[0]) &&
      idArray[0] !== EMPTY_FINGERPRINT
    ) {
      hash = (0, _util.stringToBytes)(idArray[0]);
    } else {
      hash = (0, _crypto.calculateMD5)(
  ...
  _getLinearizationPage(pageIndex) {
    ...
    return this.xref
      ...
      .then(obj => {
        if (
          (0, _primitives.isDict)(obj, 'Page') ||
          ((0, _primitives.isDict)(obj) && !obj.has('Type') && obj.has('Contents'))
        ) {
  ...
  checkFirstPage() {
    return this.getPage(0).catch(async reason => {
      if (reason instanceof _coreUtils.XRefEntryException) {
        this._pagePromises.length = 0;
        await this.cleanup();
        throw new _coreUtils.XRefParseException();
      }
    });
  }
