...
const CalRGBCS = (function CalRGBCSClosure() {
  ...
  function compensateBlackPoint(sourceBlackPoint, XYZFlat, result) {
    ...
    const XScale = (1 - X_DST) / (1 - X_SRC);
    const XOffset = 1 - XScale;
    const YScale = (1 - Y_DST) / (1 - Y_SRC);
    const YOffset = 1 - YScale;
    const ZScale = (1 - Z_DST) / (1 - Z_SRC);
    const ZOffset = 1 - ZScale;
    result[0] = XYZFlat[0] * XScale + XOffset;
    result[1] = XYZFlat[1] * YScale + YOffset;
    result[2] = XYZFlat[2] * ZScale + ZOffset;
  }

  function normalizeWhitePointToFlat(sourceWhitePoint, XYZIn, result) {
    ...
    const LMS = result;
    matrixProduct(BRADFORD_SCALE_MATRIX, XYZIn, LMS);
    const LMSFlat = tempNormalizeMatrix;
    convertToFlat(sourceWhitePoint, LMS, LMSFlat);
    matrixProduct(BRADFORD_SCALE_INVERSE_MATRIX, LMSFlat, result);
  }

  function normalizeWhitePointToD65(sourceWhitePoint, XYZIn, result) {
    const LMS = result;
    matrixProduct(BRADFORD_SCALE_MATRIX, XYZIn, LMS);
  ...
  function convertToRgb(cs, src, srcOffset, dest, destOffset, scale) {
    ...
    XYZ[1] = Y;
    XYZ[2] = Z;
    const XYZFlat = tempConvertMatrix2;
    normalizeWhitePointToFlat(cs.whitePoint, XYZ, XYZFlat);
    const XYZBlack = tempConvertMatrix1;
    compensateBlackPoint(cs.blackPoint, XYZFlat, XYZBlack);
    const XYZ_D65 = tempConvertMatrix2;
    normalizeWhitePointToD65(FLAT_WHITEPOINT_MATRIX, XYZBlack, XYZ_D65);
