...
class ChunkedStream {
  ...
  ensureByte(pos) {
    ...
    if (!this.loadedChunks[chunk]) {
      throw new _coreUtils.MissingDataException(pos, pos + 1);
    }
  ...
  ensureRange(begin, end) {
    ...
    for (let chunk = beginChunk; chunk < endChunk; ++chunk) {
      if (!this.loadedChunks[chunk]) {
        throw new _coreUtils.MissingDataException(begin, end);
      }
    }
  }
...
class ChunkedStreamManager {
  ...
  onReceiveData(args) {
    ...
    const endChunk =
      end < this.length ? Math.floor(end / this.chunkSize) : Math.ceil(end / this.chunkSize);
