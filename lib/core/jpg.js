...
let JpegImage = (function JpegImageClosure() {
      ...
      if (bitsData === 0xff) {
        ...
        if (nextByte) {
          if (nextByte === 0xdc && parseDNLMarker) {
            offset += 2;
            const scanLines = (0, _coreUtils.readUint16)(data, offset);
            offset += 2;
  ...
  function quantizeAndInverse(component, blockBufferOffset, p) {
    ...
    let blockData = component.blockData;
  ...
  function findNextFileMarker(data, currentPos, startPos = currentPos) {
    ...
    let currentMarker = (0, _coreUtils.readUint16)(data, currentPos);
    ...
    let newMarker = (0, _coreUtils.readUint16)(data, newPos);
    ...
    while (!(newMarker >= 0xffc0 && newMarker <= 0xfffe)) {
      ...
      newMarker = (0, _coreUtils.readUint16)(data, newPos);
    }
  ...
  JpegImage.prototype = {
    parse(data, { dnlScanLines = null } = {}) {
      function readDataBlock() {
        const length = (0, _coreUtils.readUint16)(data, offset);
        offset += 2;
        ...
        if (fileMarker && fileMarker.invalid) {
          (0, _util.warn)(
            'readDataBlock - incorrect length, current marker is: ' + fileMarker.invalid
          );
      ...
      let huffmanTablesDC = [];
      let fileMarker = (0, _coreUtils.readUint16)(data, offset);
      offset += 2;
      ...
      fileMarker = (0, _coreUtils.readUint16)(data, offset);
      offset += 2;
      ...
      markerLoop: while (fileMarker !== 0xffd9) {
        ...
        switch (fileMarker) {
          ...
          case 0xffdb:
            const quantizationTablesLength = (0, _coreUtils.readUint16)(data, offset);
            offset += 2;
            ...
            while (offset < quantizationTablesEnd) {
              ...
              if (quantizationTableSpec >> 4 === 0) {
                ...
                for (j = 0; j < 64; j++) {
                  z = dctZigZag[j];
                  tableData[z] = (0, _coreUtils.readUint16)(data, offset);
                  offset += 2;
                }
              } else {
          ...
          case 0xffc2:
            ...
            const sofScanLines = (0, _coreUtils.readUint16)(data, offset);
            offset += 2;
            frame.scanLines = dnlScanLines || sofScanLines;
            frame.samplesPerLine = (0, _coreUtils.readUint16)(data, offset);
            offset += 2;
            frame.components = [];
            frame.componentIds = {};
          ...
          case 0xffc4:
            const huffmanLength = (0, _coreUtils.readUint16)(data, offset);
            offset += 2;
          ...
          case 0xffdd:
            offset += 2;
            resetInterval = (0, _coreUtils.readUint16)(data, offset);
            offset += 2;
            break;
    ...
    _getLinearizedBlockData(width, height, isSourcePDF = false) {
      ...
      let offset = 0;
      let output;
      let numComponents = this.components.length;
      let dataLength = width * height * numComponents;
      let data = new Uint8ClampedArray(dataLength);
      let xScaleBlockOffset = new Uint32Array(width);
      let mask3LSB = 0xfffffff8;
    ...
    getData({ width, height, forceRGB = false, isSourcePDF = false }) {
      ...
      let data = this._getLinearizedBlockData(width, height, isSourcePDF);

      if (this.numComponents === 1 && forceRGB) {
        let dataLength = data.length;
        let rgbData = new Uint8ClampedArray(dataLength * 3);
        let offset = 0;

        for (let i = 0; i < dataLength; i++) {
          let grayColor = data[i];
