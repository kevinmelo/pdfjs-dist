...
let _jpx = require('./jpx.js');

let PDFImage = (function PDFImageClosure() {
  ...
  function PDFImage({
    ...
    if (
      ...
      for (let i = 0, j = 0; i < this.decode.length; i += 2, ++j) {
        let dmin = this.decode[i];
        let dmax = this.decode[i + 1];
    ...
    if (smask) {
      ...
      if ((0, _primitives.isStream)(mask)) {
        let maskDict = mask.dict;

        let imageMask = maskDict.get('ImageMask', 'IM');
  ...
  PDFImage.buildImage = function({
    ...
    let imagePromise = handleImageData(image, nativeDecoder);
    let smaskPromise;
    let maskPromise;
    let smask = image.dict.get('SMask');
    let mask = image.dict.get('Mask');
  ...
  PDFImage.createMask = function({
    ...
    let computedLength = ((width + 7) >> 3) * height;
    let actualLength = imgArray.byteLength;
    let haveFullData = computedLength === actualLength;
    let data, i;
  ...
  PDFImage.prototype = {
    ...
    decodeBuffer(buffer) {
      ...
      let decodeAddends = this.decodeAddends;
      let decodeCoefficients = this.decodeCoefficients;
      let max = (1 << bpc) - 1;
      let i, ii;
    ...
    getComponents(buffer) {
      ...
      let numComps = this.numComps;
      let length = width * height * numComps;
      let bufferPos = 0;
      let output;
      ...
      if (smask) {
        ...
        if (mask instanceof PDFImage) {
          ...
          for (i = 0, ii = width * height; i < ii; ++i) {
            let opacity = 0;
            let imageOffset = i * numComps;

            for (j = 0; j < numComps; ++j) {
              let color = image[imageOffset + j];
              let maskOffset = j * 2;
    ...
    undoPreblend(buffer, width, height) {
      let matte = this.smask && this.smask.matte;

      if (!matte) {
        return;
      }

      let matteRgb = this.colorSpace.getRgb(matte, 0);
      let matteR = matteRgb[0];
      let matteG = matteRgb[1];
      let matteB = matteRgb[2];
    ...
    createImageData(forceRGBA = false) {
      ...
      let bpc = this.bpc;
      let rowBytes = (originalWidth * numComps * bpc + 7) >> 3;
      let imgArray;
      ...
      if (!forceRGBA) {
        ...
        if (this.image instanceof _jpegStream.JpegStream && !this.smask && !this.mask) {
      ...
      imgArray = this.getImageBytes(originalHeight * rowBytes);
      let actualHeight = 0 | (((imgArray.length / rowBytes) * drawHeight) / originalHeight);
      let comps = this.getComponents(imgArray);
    ...
    fillGrayBuffer(buffer) {
      ...
      let height = this.height;
      let bpc = this.bpc;
      let rowBytes = (width * numComps * bpc + 7) >> 3;
      let imgArray = this.getImageBytes(height * rowBytes);
      let comps = this.getComponents(imgArray);
      let i, length;
