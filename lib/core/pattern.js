...
Shadings.Mesh = (function MeshClosure() {
  ...
  MeshStreamReader.prototype = {
    ...
    readCoordinate: function MeshStreamReader_readCoordinate() {
      ...
      var scale =
        bitsPerCoordinate < 32 ? 1 / ((1 << bitsPerCoordinate) - 1) : 2.3283064365386963e-10;
      return [
    ...
    readComponents: function MeshStreamReader_readComponents() {
      ...
      var scale =
        bitsPerComponent < 32 ? 1 / ((1 << bitsPerComponent) - 1) : 2.3283064365386963e-10;
      var decode = this.context.decode;
  ...
  function buildFigureFromPatch(mesh, index) {
    ...
    var splitXBy = Math.ceil(
      ((figureMaxX - figureMinX) * TRIANGLE_DENSITY) / (mesh.bounds[2] - mesh.bounds[0])
    );
    splitXBy = Math.max(
    ...
    var splitYBy = Math.ceil(
      ((figureMaxY - figureMinY) * TRIANGLE_DENSITY) / (mesh.bounds[3] - mesh.bounds[1])
    );
    splitYBy = Math.max(
      MIN_SPLIT_PATCH_CHUNKS_AMOUNT,
