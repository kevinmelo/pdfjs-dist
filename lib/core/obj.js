...
class Catalog {
    ...
    try {
      ...
      for (const groupRef of groupsData) {
        ...
        groups.push({
          ...
          name: (0, _util.isString)(group.get('Name'))
            ? (0, _util.stringToPDFString)(group.get('Name'))
            : null,
          intent: (0, _util.isString)(group.get('Intent'))
  ...
  _readOptionalContentConfig(config, contentGroupRefs) {
    ...
    return {
      name: (0, _util.isString)(config.get('Name'))
        ? (0, _util.stringToPDFString)(config.get('Name'))
        : null,
      creator: (0, _util.isString)(config.get('Creator'))
        ? (0, _util.stringToPDFString)(config.get('Creator'))
        : null,
      baseState: (0, _primitives.isName)(config.get('BaseState'))
  ...
  get javaScript() {
    ...
    if (
      (0, _primitives.isDict)(openAction) &&
      (0, _primitives.isName)(openAction.get('S'), 'JavaScript')
    ) {
    ...
    function next() {
      while (nodesToVisit.length) {
        ...
        if ((0, _primitives.isRef)(currentNode)) {
          ...
          xref.fetchAsync(currentNode).then(function(obj) {
            if (
              ...
              ((0, _primitives.isDict)(obj) && !obj.has('Kids'))
            ) {
        ...
        if (!(0, _primitives.isDict)(currentNode)) {
          capability.reject(
            new _util.FormatError('Page dictionary kid reference points to wrong type of object.')
          );
          return;
        }
        ...
        if (!Array.isArray(kids)) {
          if (
            (0, _primitives.isName)(currentNode.get('Type'), 'Page') ||
        ...
        while (this.startXRefQueue.length) {
          ...
          if ((0, _primitives.isCmd)(obj, 'xref')) {
            ...
            if (
              ...
              !(0, _primitives.isCmd)(parser.getObj(), 'obj') ||
...
class NameOrNumberTree {
  ...
  get(key) {
    ...
    if (Array.isArray(entries)) {
      ...
      (0, _util.info)(
        `Falling back to an exhaustive search, for key "${key}", ` + `in "${this._type}" tree.`
      );
