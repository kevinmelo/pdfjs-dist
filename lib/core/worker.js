...
class WorkerMessageHandler {
  ...
  static createDocumentHandler(docParams, port) {
    ...
    function getPdfManager(data, evaluatorOptions) {
      ...
      if (source.data) {
        try {
          newPdfManager = new _pdf_manager.LocalPdfManager(
            docId,
            source.data,
            source.password,
            evaluatorOptions,
      ...
      fullRequest.headersReady
        .then(function() {
          ...
          newPdfManager = new _pdf_manager.NetworkPdfManager(
            docId,
            pdfStream,
            {
              msgHandler: handler,
    ...
    function setupDoc(data) {
      ...
      function onFailure(ex) {
        ...
        if (ex instanceof _util.PasswordException) {
          ...
          ex instanceof _util.InvalidPDFException ||
          ex instanceof _util.MissingPDFException ||
          ex instanceof _util.UnexpectedResponseException ||
          ex instanceof _util.UnknownErrorException
        ) {
          handler.send('DocException', ex);
        } else {
        ...
        page
          ...
          .then(
            function() {
              ...
              if (start) {
                (0, _util.info)(
                  `page=${pageIndex + 1} - getTextContent: time=` + `${Date.now() - start}ms`
                );
              }
