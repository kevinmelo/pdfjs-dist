...
class WidgetAnnotation extends Annotation {
  ...
  _decodeFormValue(formValue) {
    if (Array.isArray(formValue)) {
      return formValue
        .filter(item => (0, _util.isString)(item))
        .map(item => (0, _util.stringToPDFString)(item));
  ...
  async _getAppearance(evaluator, task, annotationStorage) {
    ...
    if (alignment === 0 || alignment > 2) {
      return (
        '/Tx BMC q BT ' +
        defaultAppearance +
        ` 1 0 0 1 ${hPadding} ${vPadding} Tm (${(0, _util.escapeString)(value)}) Tj` +
        ' ET Q EMC'
      );
    }
...
class TextWidgetAnnotation extends WidgetAnnotation {
  ...
  _getMultilineAppearance(
    ...
    return (
      '/Tx BMC q BT ' + defaultAppearance + ` 1 0 0 1 0 ${height} Tm ${renderedText}` + ' ET Q EMC'
    );
  }
