...
let ARCFourCipher = (function ARCFourCipherClosure() {
...
let calculateMD5 = (function calculateMD5Closure() {
...
let Word64 = (function Word64Closure() {
  ...
  Word64.prototype = {
    and: function Word64And(word) {
      this.high &= word.high;
      this.low &= word.low;
    },
    xor: function Word64Xor(word) {
      this.high ^= word.high;
      this.low ^= word.low;
    },
    or: function Word64Or(word) {
      this.high |= word.high;
      this.low |= word.low;
    },
    shiftRight: function Word64ShiftRight(places) {
      if (places >= 32) {
    ...
    shiftLeft: function Word64ShiftLeft(places) {
      if (places >= 32) {
    ...
    rotateRight: function Word64RotateRight(places) {
      let low, high;
    ...
    not: function Word64Not() {
      this.high = ~this.high;
      this.low = ~this.low;
    },
    add: function Word64Add(word) {
      let lowAdd = (this.low >>> 0) + (word.low >>> 0);
    ...
    copyTo: function Word64CopyTo(bytes, offset) {
    ...
    assign: function Word64Assign(word) {
      this.high = word.high;
      this.low = word.low;
    }
  };
...
let calculateSHA256 = (function calculateSHA256Closure() {
  function rotr(x, n) {
    return (x >>> n) | (x << (32 - n));
  }
...
let calculateSHA512 = (function calculateSHA512Closure() {
  ...
  function hash(data, offset, length, mode384) {
    ...
    let paddedLength = Math.ceil((length + 17) / 128) * 128;
    let padded = new Uint8Array(paddedLength);
    let i, j, n;
    ...
    let t1 = new Word64(0, 0);

    let t2 = new Word64(0, 0);
    let tmp1 = new Word64(0, 0);

    let tmp2 = new Word64(0, 0);
...
let calculateSHA384 = (function calculateSHA384Closure() {
  function hash(data, offset, length) {
...
let NullCipher = (function NullCipherClosure() {
  function NullCipher() {}
...
let PDF17 = (function PDF17Closure() {
  ...
  PDF17.prototype = {
    checkOwnerPassword: function PDF17CheckOwnerPassword(
      password,
      ownerValidationSalt,
      userBytes,
      ownerPassword
    ) {
      let hashData = new Uint8Array(password.length + 56);
      hashData.set(password, 0);
      hashData.set(ownerValidationSalt, password.length);
      hashData.set(userBytes, password.length + ownerValidationSalt.length);
      let result = calculateSHA256(hashData, 0, hashData.length);
      return compareByteArrays(result, ownerPassword);
    },
    checkUserPassword: function PDF17CheckUserPassword(password, userValidationSalt, userPassword) {
      let hashData = new Uint8Array(password.length + 8);
      hashData.set(password, 0);
      hashData.set(userValidationSalt, password.length);
      let result = calculateSHA256(hashData, 0, hashData.length);
      return compareByteArrays(result, userPassword);
    },
    getOwnerKey: function PDF17GetOwnerKey(password, ownerKeySalt, userBytes, ownerEncryption) {
      let hashData = new Uint8Array(password.length + 56);
      hashData.set(password, 0);
      hashData.set(ownerKeySalt, password.length);
      hashData.set(userBytes, password.length + ownerKeySalt.length);
      let key = calculateSHA256(hashData, 0, hashData.length);
      let cipher = new AES256Cipher(key);
      return cipher.decryptBlock(ownerEncryption, false, new Uint8Array(16));
    },
    getUserKey: function PDF17GetUserKey(password, userKeySalt, userEncryption) {
      let hashData = new Uint8Array(password.length + 8);
      hashData.set(password, 0);
...
let PDF20 = (function PDF20Closure() {
  function concatArrays(array1, array2) {
    let t = new Uint8Array(array1.length + array2.length);
    t.set(array1, 0);
  ...
  function calculatePDF20Hash(password, input, userBytes) {
    ...
    while (i < 64 || e[e.length - 1] > i - 32) {
      let arrayLength = password.length + k.length + userBytes.length;
      let k1 = new Uint8Array(arrayLength * 64);
      ...
      let cipher = new AES128Cipher(k.subarray(0, 16));
  ...
  PDF20.prototype = {
    hash: function PDF20Hash(password, concatBytes, userBytes) {
      return calculatePDF20Hash(password, concatBytes, userBytes);
    },
    checkOwnerPassword: function PDF20CheckOwnerPassword(
      password,
      ownerValidationSalt,
      userBytes,
      ownerPassword
      ...
      hashData.set(ownerValidationSalt, password.length);
      hashData.set(userBytes, password.length + ownerValidationSalt.length);
      let result = calculatePDF20Hash(password, hashData, userBytes);
      return compareByteArrays(result, ownerPassword);
    },
    checkUserPassword: function PDF20CheckUserPassword(password, userValidationSalt, userPassword) {
      let hashData = new Uint8Array(password.length + 8);
      hashData.set(password, 0);
      hashData.set(userValidationSalt, password.length);
      let result = calculatePDF20Hash(password, hashData, []);
      return compareByteArrays(result, userPassword);
    },
    getOwnerKey: function PDF20GetOwnerKey(password, ownerKeySalt, userBytes, ownerEncryption) {
      let hashData = new Uint8Array(password.length + 56);
      hashData.set(password, 0);
      hashData.set(ownerKeySalt, password.length);
      hashData.set(userBytes, password.length + ownerKeySalt.length);
      let key = calculatePDF20Hash(password, hashData, userBytes);
      let cipher = new AES256Cipher(key);
      return cipher.decryptBlock(ownerEncryption, false, new Uint8Array(16));
    },
    getUserKey: function PDF20GetUserKey(password, userKeySalt, userEncryption) {
      let hashData = new Uint8Array(password.length + 8);
      hashData.set(password, 0);
      hashData.set(userKeySalt, password.length);
      let key = calculatePDF20Hash(password, hashData, []);
      let cipher = new AES256Cipher(key);
      return cipher.decryptBlock(userEncryption, false, new Uint8Array(16));
    }
  };
...
let CipherTransform = (function CipherTransformClosure() {
  ...
  CipherTransform.prototype = {
    createStream: function CipherTransformCreateStream(stream, length) {
      let cipher = new this.StreamCipherConstructor();
      return new _stream.DecryptStream(stream, length, function cipherTransformDecryptStream(
        data,
        finalize
      ) {
    ...
    decryptString: function CipherTransformDecryptString(s) {
      let cipher = new this.StringCipherConstructor();
...
let CipherTransformFactory = (function CipherTransformFactoryClosure() {
  let defaultPasswordBytes = new Uint8Array([
    0x28,
    0xbf,
    0x4e,
    0x5e,
  ...
  function createEncryptionKey20(
    ...
    if (password) {
      let passwordLength = Math.min(127, password.length);
  ...
  function prepareKeyData(
    ...
    let hashDataSize = 40 + ownerPassword.length + fileId.length;
    ...
    let keyLengthInBytes = keyLength >> 3;
    ...
    let encryptionKey = hash.subarray(0, keyLengthInBytes);
  ...
  let identityName = _primitives.Name.get('Identity');
  ...
  function CipherTransformFactory(dict, fileId, password) {
    ...
    if (!keyLength) {
      if (algorithm <= 3) {
        ...
        let cfDict = dict.get('CF');
        let streamCryptoName = dict.get('StmF');

        if ((0, _primitives.isDict)(cfDict) && (0, _primitives.isName)(streamCryptoName)) {
          cfDict.suppressEncryption = true;
          let handlerDict = cfDict.get(streamCryptoName.name);
    ...
    let ownerPassword = (0, _util.stringToBytes)(dict.get('O')).subarray(0, 32);
    let userPassword = (0, _util.stringToBytes)(dict.get('U')).subarray(0, 32);
    let flags = dict.get('P');
    let revision = dict.get('R');
    let encryptMetadata =
      (algorithm === 4 || algorithm === 5) && dict.get('EncryptMetadata') !== false;
    this.encryptMetadata = encryptMetadata;
    let fileIdBytes = (0, _util.stringToBytes)(fileId);
    let passwordBytes;
    ...
    if (algorithm !== 5) {
      ...
      let ownerValidationSalt = (0, _util.stringToBytes)(dict.get('O')).subarray(32, 40);
      let ownerKeySalt = (0, _util.stringToBytes)(dict.get('O')).subarray(40, 48);
      let uBytes = (0, _util.stringToBytes)(dict.get('U')).subarray(0, 48);
      let userValidationSalt = (0, _util.stringToBytes)(dict.get('U')).subarray(32, 40);
      let userKeySalt = (0, _util.stringToBytes)(dict.get('U')).subarray(40, 48);
      let ownerEncryption = (0, _util.stringToBytes)(dict.get('OE'));
      let userEncryption = (0, _util.stringToBytes)(dict.get('UE'));
      let perms = (0, _util.stringToBytes)(dict.get('Perms'));
    ...
    if (!encryptionKey && !password) {
      ...
      let decodedPassword = decodeUserPassword(passwordBytes, ownerPassword, revision, keyLength);
      encryptionKey = prepareKeyData(
        fileIdBytes,
        decodedPassword,
        ownerPassword,
  ...
  function buildCipherConstructor(cf, name, num, gen, key) {
    ...
    let cryptFilter = cf.get(name.name);
    let cfm;
  ...
  CipherTransformFactory.prototype = {
    createCipherTransform: function CipherTransformFactoryCreateCipherTransform(num, gen) {
      ...
      let key = buildObjectKey(num, gen, this.encryptionKey, false);

      let cipherConstructor = function buildCipherCipherConstructor() {
